...
(function(window, $, undefined) {
    ...
    $.html5imageupload1 = function html5imageupload1(options, element) {

        this.element = element;

        this.options = $.extend(true, {}, $.html5imageupload1.defaults, options, $(this.element).data());
        ...
        this.button.edit = '<div class="btn btn-dark btn-icon"><i class="la la-edit"></i></div>'

        this.button.saving = '<div class="btn btn-warning btn-sm">' + (this.options.saveLabel || 'Saving...') + ' <i class="la la-clock-o"></i></div>';

        this.button.zoomin = '<div class="btn btn-dark btn-icon"><i class="la la-expand"></i></div>';

        this.button.zoomout = '<div class="btn btn-dark btn-icon"><i class="la la-compress"></i></div>';

        this.button.zoomreset = '<div class="btn btn-dark btn-icon"><i class="la la-arrows"></i></div>';

        this.button.cancel = '<div class="btn btn-danger btn-icon"><i class="la la-remove"></i></div>';

        this.button.done = '<div class="btn btn-success btn-icon"><i class="la la-check"></i></div>';

        this.button.del = '<div class="btn btn-danger btn-icon"><i class="la la-trash-o"></i></div>';
    ...
    $.html5imageupload1.prototype = {
        ...
        readImage: function(image, src, name, mimeType) {
            ...
            img.onload = function(tmp) {
                ...
                if (parseInt(useWidth1) < custom_img_width1) {
        ...
        imageCrop: function() {
            ...
            if (options.canvas == true) {
                ...
                var canvas = $('<canvas class="final" id="canvas_' + $(input).attr('name') + '" width="' + finalWidth + '" height="' + finalHeight + '" style="position:absolute; top: 0; bottom: 0; left: 0; right: 0; z-index:90; width: 100%; height: 100%;"></canvas>');
    ...
    $.fn.html5imageupload1 = function(options) {

        if ($.data(this, "html5imageupload1")) return;

        return $(this).each(function() {

            new $.html5imageupload1(options, this);

            $.data(this, "html5imageupload1");
