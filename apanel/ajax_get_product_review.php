...
if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
	$ctable_where .= " (
                            pro_rating like '%".$_REQUEST['searchName']."%' OR
                            user_id like (select id from user where first_name like '%".$_REQUEST['searchName']."%') OR
                            user_id like (select id from user where last_name like '%".$_REQUEST['searchName']."%') OR
                            review_desc like '%".$_REQUEST['searchName']."%' 
						) AND ";
}
    ...
    if(@mysqli_num_rows($ctable_r)>0){
        ...
        <tr>
            ...
            <td class="text-center">
                <?php 
                echo $db->getStar($ctable_d['pro_rating'],1);
                ?>
            </td>
            <td><?=stripslashes($ctable_d['review_desc']);?></td>
