...
$main_page 			= "brand-price-increase"; //for sidebar active menu
$page_title 		= ucwords($_REQUEST['mode'])." ".$ctable1;
$manage_page_url 	= ADMINURL."brand-price-increase/";
...
if(isset($_REQUEST['submit']))
{
	...
	$where_pro 		= " pro_feature_brand='".$pro_feature_brand."' AND isDelete=0";
	$ctable_pro_r 	= $db->rpgetData($ctable,"*",$where_pro);
	//$ctable_pro_d 	= @mysqli_fetch_array($ctable_pro_r);


	if($pro_feature_brand == 0)
	{
		$update = "UPDATE product SET price = ROUND(price + (price * ".$amount." / 100),2) WHERE isDelete=0";
		/*echo $update;die;*/
		$rows 	= array(
					"pro_feature_brand",
					"amount_percentage",
	...
	{
		$update = "UPDATE product SET price = ROUND(price + (price * ".$amount." / 100),2) WHERE pro_feature_brand=".$pro_feature_brand." AND isDelete=0";
		/*echo $update;die;*/
		$rows 	= array(
					"pro_feature_brand",
					"amount_percentage",
		...
		/*if(@mysqli_num_rows($ctable_pro_r)>0)
		{
			...
			while($ctable_pro_d = @mysqli_fetch_array($ctable_pro_r))
			{
				...
				$where = "isDelete=0 AND pro_feature_brand='".$pro_feature_brand."' AND id='".$id."' ";
				$db->rpupdate($ctable,$rows,$where);
				$price = 0;
			}
		}*/
										    ...
										    <div class="kt-portlet__body">
												...
												<div class="form-group">
													...
													<select class="form-control" name="pro_feature_brand" id="pro_feature_brand">
														...
														<option value="0" >For All Brand</option>
														<?php
														$cate_r = $db->rpgetData("featured_brand","*","isDelete=0", "name");
														if(@mysqli_num_rows($cate_r)>0){
															while($cate_d = @mysqli_fetch_array($cate_r)){
															?>
															<option value="<?php echo $cate_d['id']; ?>" <?php if($cate_d['id']==$pro_feature_brand){?> selected <?php } ?>><?php echo $cate_d['name']; ?></option>
															<?php
															}
														}
														?>
        ...
        <script type="text/javascript">
        	$(function(){
				$("#frm").validate({
					ignore: "",
					rules: {
						pro_feature_brand:{required : true},
		    			amount:{required : true},
					},
					messages: {
						pro_feature_brand:{required:"Please select brand."},
			...
			$(document).ready(function(){
				...
				<?php unset($_SESSION['MSG']); } else if(isset($_SESSION['MSG']) && $_SESSION['MSG'] == 'Single_Category') { ?>
