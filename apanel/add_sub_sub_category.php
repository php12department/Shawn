...
$ctable1 			= "Sub Sub Category";
$parent_page 		= "product-master"; //for sidebar active menu
$main_page 			= "manage-sub-sub-category"; //for sidebar active menu
$page_title 		= ucwords($_REQUEST['mode'])." ".$ctable1;
$manage_page_url 	= ADMINURL."manage-sub-sub-category/";
$add_page_url 		= ADMINURL."add-sub-sub-category/add/";
$edit_page_url 		= ADMINURL."add-sub-sub-category/edit/".$_REQUEST['id']."/";
...
if(isset($_REQUEST['submit']))
{
	...
	if(isset($_FILES['banner_image_path']) && !empty($_FILES['banner_image_path']['name']))   
	{
		...
		else
		{
            ...
            {
                $_SESSION['MSG'] = "FILE_TYPE_ERR";
                $_SESSION['FILE_TYPE_ERR_MSG'] = "Please upload video instead of video.";
		...
		$banner_image_path = time()."-subsubcate-".$img_filename;
	...
	{    
		$dup_where = "slug = '".$slug."' AND cate_id='".$cate_id."' AND sub_cate_id='".$sub_cate_id."' AND  id!='".$_REQUEST['id']."' AND isDelete=0";
		$r = $db->rpdupCheck($ctable,$dup_where);
		if($r)
		{
			$_SESSION['MSG'] = "Duplicate";
