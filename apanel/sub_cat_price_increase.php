...
$main_page 			= "sub-cat-price-increase"; //for sidebar active menu
$page_title 		= ucwords($_REQUEST['mode'])." ".$ctable1;
$manage_page_url 	= ADMINURL."sub-cat-price-increase/";
...
if(isset($_REQUEST['submit']))
{
	...
	$where_pro 		= " sub_cate_id='".$sub_cate_id."' AND isDelete=0";
	...
	if($sub_cate_id == 0)
	{
		$update = "UPDATE product SET price = ROUND(price + (price * ".$amount." / 100),2) WHERE cate_id=".$cate_id." AND isDelete=0";
		/*echo $update;die;*/
		$rows 	= array(
					"cate_id",
					"sub_cate_id",
	...
	{
		$update = "UPDATE product SET price = ROUND(price + (price * ".$amount." / 100),2) WHERE sub_cate_id=".$sub_cate_id." AND isDelete=0";
		/*echo $update;die;*/
		$rows 	= array(
					"sub_cate_id",
					"amount_percentage",
		...
		/*if(@mysqli_num_rows($ctable_pro_r)>0)
		{
			...
			while($ctable_pro_d = @mysqli_fetch_array($ctable_pro_r))
			{
				...
				$where = "isDelete=0 AND sub_cate_id='".$sub_cate_id."' AND id='".$id."' ";
				$db->rpupdate($ctable,$rows,$where);
				$price = 0;
			}
		}*/
    ...
    <body  class="kt-quick-panel--right kt-demo-panel--right kt-offcanvas-panel--right kt-header--fixed kt-header-mobile--fixed kt-subheader--enabled kt-subheader--fixed kt-subheader--solid kt-aside--enabled kt-aside--fixed kt-page--loading"  >
										    ...
										    <div class="kt-portlet__body">
												...
												<div class="form-group">
													...
													<select class="form-control" name="cate_id" id="cate_id" onChange="getSubCat(this.value);">
														...
														<?php
														$cate_r = $db->rpgetData("category","*","isDelete=0", 'name');
														if(@mysqli_num_rows($cate_r)>0){
															while($cate_d = @mysqli_fetch_array($cate_r)){
															?>
															<option value="<?php echo $cate_d['id']; ?>"><?php echo $cate_d['name']; ?></option>
															<?php
															}
														}
														?>
        ...
        <script type="text/javascript">
			...
			$(document).ready(function(){
				...
				<?php unset($_SESSION['MSG']); } else if(isset($_SESSION['MSG']) && $_SESSION['MSG'] == 'All_Sub_Category') { ?>
				 	$.notify({message: 'Price Increase For All Sub Category Successfully.' },{type: 'success'});
