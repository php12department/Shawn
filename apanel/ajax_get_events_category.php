...
if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
	$ctable_where .= " (
                            cat_name like '%".$_REQUEST['searchName']."%'   
						) AND ";
}
...
<table id="kt_table_1" class="table table-striped- table-bordered table-hover table-checkable">
    ...
    if(@mysqli_num_rows($ctable_r)>0)
    {
	    ...
	    <tr>
	        ...
	        <td><?php echo stripslashes($ctable_d['cat_name']); ?></td>
	        <td>
                <a href="<?php echo ADMINURL?>add-<?php echo str_replace("_", "-", "events-news-category"); ?>/edit/<?php echo $ctable_d['id']; ?>/" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit">
